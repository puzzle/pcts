...
//
// assertTrue(exception.getError().message().contains("Uploaded file is
// empty"));
// }
//
// @Test
// void testParseToPromptText_IOExceptionThrowsMigrationException() throws
// IOException {
// MultipartFile mockFile = mock(MultipartFile.class);
...
// () -> odsParserService.parseToPromptText(mockFile, tableNames));
//
// assertTrue(exception.getError().message().contains("Failed to parse ODS file:
// Stream error"));
// }
//
// @Test
...
// MultipartFile mockFile = mock(MultipartFile.class);
// when(mockFile.isEmpty()).thenReturn(false);
// when(mockFile.getInputStream()).thenReturn(new ByteArrayInputStream(new
// byte[0]));
//
...
//
// try (MockedStatic<OdfSpreadsheetDocument> mockedStatic =
// mockStatic(OdfSpreadsheetDocument.class)) {
// mockedStatic.when(() ->
// OdfSpreadsheetDocument.loadDocument(any(InputStream.class))).thenReturn(mockDoc);
//
// MigrationException exception = assertThrows(MigrationException.class,
// () -> odsParserService.parseToPromptText(mockFile, tableNames));
//
