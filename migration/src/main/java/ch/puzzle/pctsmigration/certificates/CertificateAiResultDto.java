...
public record CertificateAiResultDto(
        @NotNull
        @NotBlank
        @JsonPropertyDescription("This attribute is the name of a certificate. It always appears in the 'Zertifikat' column.")
        String name,
        ...
        @JsonPropertyDescription("When processing the provided dataset to determine the point value of a certificate, please interpret the scoring columns as a binary indicator system. \n" +
                ...
                "          if a row features a '1' within the column labeled \"0.5\", you must evaluate that specific certificate as being worth exactly 0.5 points, recognizing that the other point-category columns for that same row will not contain the active '1' indicator.")
        BigDecimal points
) {
}
