...
public class LeadershipExperienceValidationService {
    ...
    @Autowired
    public LeadershipExperienceValidationService(LeadershipExperiencePersistenceService persistenceService) {
    ...
    private void validateCertificateType(CertificateType certificateType) {
        ...
        if (!List
                .of(CertificateType.LEADERSHIP_TRAINING,
                    CertificateType.MILITARY_FUNCTION,
                    CertificateType.YOUTH_AND_SPORT)
                .contains(certificateType)) {
            throw new PCTSException(HttpStatus.BAD_REQUEST,
                                    "The certificate type is not a leadership experience.",
