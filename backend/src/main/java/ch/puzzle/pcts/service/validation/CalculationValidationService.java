...
public class CalculationValidationService extends ValidationBase<Calculation> {
    ...
    public void validateUserOnlyHasOneActiveCalculationPerRole(Calculation calculation, Long id){
        List<Calculation> calculations = persistenceService.getAllByMemberIdAndRoleIdAndState(calculation.getMember().getId(), calculation.getRole().getId(), CalculationState.ACTIVE);
        if (!calculations.isEmpty() && (id == null || !id.equals(calculations.getFirst().getId()))){
            Map<FieldKey, String> attributes = Map
                    .of(FieldKey.ENTITY, CALCULATION, FieldKey.FIELD, "member", FieldKey.IS, CalculationState.ACTIVE.toString());
