...
SELECT
    ...
    -- Certificate row (0..1 per certificate_type per row)
...
-- ------------- CERTIFICATES + TAGS -------------------
    ...
    FROM certificate cert
             JOIN certificate_type ct
                  ON ct.id = cert.certificate_type_id
             LEFT JOIN certificate_type_tag ctt
                       ON ctt.certificate_type_id = ct.id
             LEFT JOIN tag t
                       ON t.id = ctt.tag_id
...
-- ------------- LEADERSHIP EXPERIENCE -------------------
    ...
    FROM certificate leader
             JOIN certificate_type lct
                  ON lct.id = leader.certificate_type_id
